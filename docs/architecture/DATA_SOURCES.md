...
| **CTA Bus Tracker (BusTime v2)** `ctabustime.com/bustime/api/v2` (`getvehicles`, `getpredictions`) + **CTA Train Tracker** `lapi.transitchicago.com/api/1.0` (`ttpositions`, `ttarrivals`) | mapping (live) | **live vehicle positions + next-arrival predictions** per route/stop, normalized to `{vehicles[], arrivals[]}` and joined to e3a `gold_routes.route_id` / `gold_stops.stop_id` (authority `cta`) | **API key (free)** — server-side only: `CTA_BUS_TRACKER_API`, `CTA_TRAIN_TRACKER_API` (never in client bundle) | live (endpoint, cached ~15s) / sampled (E11 append log) | public (CTA developer terms; free tier, daily call caps) | ☐ (v3.4 — served by `/api/live/<metro>`; keys server-side; E11 samples → append-only NDJSON under `data/bronze/<metro>/cta_live/`) |
| **CTA live-feed delay + crowding proxy** (derived, v3.e11) — SourceTag id `delays` | mapping (live) | rolls the E11 sample log into: **delayed share** by mode (CTA's own `delayed` flag — NOT a schedule deviation; the prediction feed has no scheduled time), **next-arrival wait distribution**, and a **bunching** crowding proxy (two same-route vehicles predicted at one stop within 2 min = collapsed headway). No passenger-load data on the free feed | none (derived from the row above) | sampled (rebuilt by `pipeline/delays.py` from the append log) | derived (inherits CTA developer terms) | ☐ (v3.e11 — `pipeline/delays.py` → `frontend/src/data/<slug>/delays.json`; honest DataState until samples accumulate) |
