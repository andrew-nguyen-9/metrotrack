...
|---|---|---|---|---|
| v2.0 | Multi-tenant foundation | done | — | **v2.0 closed.** v2.0.6 done: per-metro tiles (`public/<slug>/transit.pmtiles` + size cap), `verify_metro` integrity gate, nightly per-metro matrix (`fail-fast:false`, verify-before-publish), selftest. GATE green: selftest 35, dbt 65, astro check 0, `verify_metro('chicago')` PASS, golden test on local prod artifact (a11y 100, map renders, `transit.json` byte-identical = v1 parity). **Carryover (not a blocker):** Vercel SSO 302s headless requests so the *preview* Lighthouse runs on the byte-identical local artifact (per v2.0.5); disabling Deployment Protection / adding a bypass token is a v2.1 ops item |
...
- 2026-06-28 · v2.0.6 · done · **closes v2.0.** tiles.py per-metro (`public/<slug>/transit.pmtiles` + `src/data/<slug>/transit.json`, every query metro_id-filtered, 50 MB cap assert [I4]); `pipeline/checks.py` `verify_metro` integrity gate (figure→bronze receipt + agency coverage + null-metro_id + operator freshness floor); nightly.yml → discover→matrix(`fail-fast:false`, `max-parallel:1`, verify-before-publish)→deploy [H11a,H12a,I5a]; selftest +5 verify fixtures. GATE: selftest 35, dbt 65, astro check 0, advisors N/A (no schema change), verify_metro('chicago') PASS, golden test on local prod artifact (a11y 100, map renders per-slug pmtiles, transit.json byte-identical=parity). Vercel-preview Lighthouse still SSO-blocked → v2.1 ops · v2.0.6
