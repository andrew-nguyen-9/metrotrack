...
-- ── TOD pillar: central business districts + per-hex TOD mart [v3.10] ────
-- CBDs are data-driven + multi-district: metros/<slug>.toml declares N anchors,
-- so adding a district is a config row, not a schema change. Point geometry (4326).
create table if not exists public.cbds (
  metro_id    text not null references public.metros(metro_id),
  cbd_id      text not null,                     -- kebab slug, unique within a metro
  name        text not null,
  geom        extensions.geometry(Point, 4326),
  as_of       date,
...
-- Per-hex transit-oriented-development mart: density (jobs/pop), growth vs the prior
-- vintage (signed %, null where the prior cell had none), and straight-line distance
-- + best-case minutes to the nearest configured CBD (ORS refines when keyed).
